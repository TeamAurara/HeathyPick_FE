...
const createMockWeeklyData = () => {
  ...
  last7Days.forEach((date, index) => {
    ...
    console.log(`목업 데이터 생성 - ${dateKey}:`, {
      ...
      total: parseInt(breakfast[0].calories) + parseInt(lunch[0].calories) + parseInt(dinner[0].calories)
    });
  });
...
export default function StatisticsScreen() {
  ...
  const loadWeeklyData = useCallback(() => {
    ...
    // 평균 계산
    const avgCalories = caloriesData.reduce((sum, cal) => sum + cal, 0) / caloriesData.length;
      ...
      <ScrollView className="flex-1 bg-gray-50">
        ...
        <WeeklyStatsChart 
          data={{
            ...
            datasets: [
              {
                data: weeklyCalories.length > 0 ? weeklyCalories : [0, 0, 0, 0, 0, 0, 0],
              },
            ],
          }}
        />
              ...
              <View className="flex-row items-center">
                <View className="w-24 bg-gray-200 rounded-full h-2 mr-2">
                  <View 
                    ...
                    style={{ 
                      width: `${Math.min((averageCalories / 1500) * 100, 100)}%` 
                    }} 
                  />
                </View>
